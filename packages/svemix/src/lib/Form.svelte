<script lang="ts">
	...
	$: if (typeof window === 'undefined' && $page.query.toString().length > 0) {
	...
	function pageQueryToFormState() {
		const errors = $page.query.getAll('errors[]') ?? [];
		const data = $page.query.getAll('data[]') ?? [];
		const formError = $page.query.get('formError') ?? '';
